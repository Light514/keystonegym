        ...
        {coachKeys.map((key, i) => {
          ...
          return (
            <motion.div
              ...
              className="col-span-1 sm:col-span-3 md:col-span-3 min-h-[350px] sm:min-h-[400px] relative group overflow-hidden border border-zinc-800 cursor-pointer"
            >
              {/* Image Background */}
              <div className="absolute inset-0 bg-zinc-900">
                <img
                  src={coachImages[key]}
                  alt={t(`list.${key}.name`)}
                  className={`w-full h-full object-cover object-top transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                    isActive
                      ? 'grayscale-0 opacity-100 scale-110'
                      : 'opacity-60 grayscale group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-110'
                  }`}
                />
                <div className={`absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent transition-opacity duration-500 ${
                  isActive ? 'opacity-70' : 'opacity-90 group-hover:opacity-70'
                }`} />
              </div>

              {/* Content Overlay */}
              <div className="absolute inset-0 p-6 flex flex-col justify-end">
                <div className={`relative z-10 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                  ...
                  <div className="flex justify-between items-end border-b border-[#D4AF37]/50 pb-4 mb-4">
                    ...
                    <Globe className={`w-4 h-4 text-[#D4AF37] transition-transform duration-500 ${
                  ...
                  <div className={`grid grid-cols-2 gap-y-2 mt-4 font-mono text-xs text-zinc-300 transition-opacity duration-500 delay-100 ${
              ...
              {/* Corner Accents */}
              <div className={`absolute top-0 left-0 w-4 h-4 border-l-2 border-t-2 border-[#D4AF37] transition-opacity duration-300 ${
                isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
              }`} />
              <div className={`absolute bottom-0 right-0 w-4 h-4 border-r-2 border-b-2 border-[#D4AF37] transition-opacity duration-300 ${
                isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
              }`} />
            </motion.div>
          );
